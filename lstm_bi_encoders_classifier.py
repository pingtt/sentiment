...
if args.select == 'train' or args.select == 'all':
    ...
    encoder = LSTM(enc_units, return_sequences=True, return_state=True)
    ...
    # merge
    merged = Concatenate(axis=1)([encoder_outputs, decoder_outputs])
    conv = Conv1D(filters=32, kernel_size=3, activation='relu')(merged)
    pool = MaxPooling1D(pool_size=2)(conv)
    flat = Flatten()(pool)
    #flat = Flatten()(merged)
    dense = Dense(32, activation='relu')(flat)
    drop = Dropout(0.2)(dense)
    dense2 = Dense(8, activation='relu')(drop)

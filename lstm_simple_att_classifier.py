...
from keras.layers import Input, Dense, Flatten, Dot, LSTM, Conv1D, MaxPooling1D, BatchNormalization
...
if args.select == 'train' or args.select == 'all':
    ...
    # print(tensorflow.shape(encoder_outputs))
    ...
    conv = Conv1D(filters=32, kernel_size=3, activation='relu')(join_outputs)
    bn = BatchNormalization()(conv)
    pool = MaxPooling1D(pool_size=2)(bn)
    flat = Flatten()(pool)
    dense = Dense(16, activation='relu')(flat)

    # flat = Flatten()(join_outputs)
    # dense = Dense(32, activation='relu')(flat)
    outputs = Dense(n_output, activation='softmax')(dense)
